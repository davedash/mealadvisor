...
class ReviewForm(forms.Form):
    # rating = forms.ChoiceField(choices=STARS,
    # error_messages={'required': 'Please choose a star rating'})
    note = forms.CharField(widget=forms.Textarea(),
    error_messages={'required': 'Please type in your message'})
...
class NewMealForm(forms.Form):
    ...
    error_messages={'required': 'Please enter the name of your dish'})
    description = forms.CharField(
    help_text="Note: This is not a review.", widget=forms.Textarea,
    required=False)
    ...
    def clean_name(self):
        ...
        if name:
            qset = (Q(name=name) | Q(slug=slugify(name))) & \
            ...
            if results:
                ...
                """This dish <a href="%s">already exists</a>""" \
                % results[0].get_absolute_url())
...
class NewRestaurantForm(forms.Form):
    ...
    error_messages={'required': 'Please enter the name of the restaurant'})
    description     = forms.CharField(
    help_text="Note: This is not a review.", widget=forms.Textarea)
    url             = forms.URLField(required=False)

    help_text = """<em>Optional</em>: Nickname for location if not
    <em>City, State</em> (e.g. Santana Mall)"""
    ...
    def clean_restaurant_name(self):
        ...
        # see if this restaurant exists or a restaurant with the same slug
        # exists
        if name:
            qset = (
                Q(name=name) |
            ...
            if results:
                raise forms.ValidationError(
                """This restaurant <a href="%s">already exists</a>.""" %\
                 results[0].get_absolute_url())
    ...
    def clean(self):
        ...
        not (address and city and state and zipcode):
            msg = 'Please give us address, city, state and zip for the ' \
            + 'location.'
