...
help="downloads database from production servers")
parser.add_option("-T", "--notransform", action="store_false",
help="does not do db import and transforms")
...
if options.download:
    ...
    # ssh root@reviewsby.us mysqldump -u migrator -pnarnians rbu --skip-opt
    # --create-options --add-drop-table --skip-add-locks  --skip-comments
    # --extended-insert > /tmp/db.rbu

    p = Popen("ssh root@reviewsby.us mysqldump -u migrator -pnarnians rbu "
    + "--skip-opt --create-options --add-drop-table --skip-add-locks "
    + "--skip-comments --extended-insert ", shell=True, stdout=PIPE)
...
SELECT
    id, replace(username, 'http://', ''),
    CONCAT(algorithm,'$', salt, '$', password), 1, last_login, created_at
FROM sf_guard_user;
...
INSERT INTO
    restaurant_tag2 (user_id, restaurant_id, created_at, tag, normalized_tag)
SELECT
    user_id, restaurant_id, created_at, tag, normalized_tag
FROM restaurant_tag ORDER BY created_at;
...
ALTER TABLE restaurant_version
    MODIFY COLUMN description TEXT CHARACTER SET latin1;
ALTER TABLE restaurant_version
    MODIFY COLUMN description BLOB;
ALTER TABLE restaurant_version
    MODIFY COLUMN description TEXT CHARACTER SET utf8;
ALTER TABLE restaurant_version
    MODIFY COLUMN html_description TEXT CHARACTER SET latin1;
ALTER TABLE restaurant_version MODIFY COLUMN html_description BLOB;
ALTER TABLE restaurant_version
    MODIFY COLUMN html_description TEXT CHARACTER SET utf8;


SET NAMES utf8;

update menu_item set name = replace(name,unhex('C3A2E282ACE2809D'), '-')
    WHERE name REGEXP UNHEX('C3A2E282ACE2809D');
update menu_item set name = replace(name,unhex('C3A2E282ACC593'), '"')
    WHERE name REGEXP UNHEX('C3A2E282ACC593');
update menu_item set name = replace(name,unhex('C3A2E282ACC29D'), '"')
    WHERE name REGEXP UNHEX('C3A2E282ACC29D');

update menuitem_version
    set html_description =
        replace(html_description,unhex('C3A2E282ACE2809D'), '-')
    WHERE html_description REGEXP UNHEX('C3A2E282ACE2809D');
update menuitem_version
    set html_description =
        replace(html_description,unhex('C3A2E282ACC593'), '"')
    WHERE html_description REGEXP UNHEX('C3A2E282ACC593');
update menuitem_version
    set html_description =
        replace(html_description,unhex('C3A2E282ACC29D'), '"')
    WHERE html_description REGEXP UNHEX('C3A2E282ACC29D');
update menuitem_version
    set html_description =
        replace(html_description,unhex('C3A2E282ACE284A2'), "'")
    WHERE html_description REGEXP UNHEX('C3A2E282ACE284A2');

update menu_item
set name=replace(name,'Ã»','û')   where name LIKE '%Ã»%';

update menu_item
set name=replace(name,'â€”','-') where name LIKE '%â€”%';

update restaurant
set name=replace(name,'Ã»','û') where name LIKE '%Ã»%';

update menu_item
set name=replace(name,'Ã©','é') where name LIKE '%Ã©%';
...
update menu_item
set name=replace(name, 'Ã¶', 'ö') where name LIKE '%Ã¶%';

update restaurant
set name=replace(name, 'Ã¶', 'ö') where name LIKE '%Ã¶%';

update menu_item
set name=replace(name, 'Ã±', 'ñ') where name LIKE '%Ã±%';

update menu_item  set name=replace(name, 'â€™', "'")
    where name like '%â€™%';
update restaurant set name=replace(name, 'â€™', "'")
    where name like '%â€™%';
update restaurant set name=replace(name, 'Ã¼', 'ü')
    where name like '%Ã¼%';
update restaurant set name=replace(name, 'TequilerÃ', 'Tequilerí')
    where name LIKE '%TequilerÃ%';
update restaurant set name=replace(name, 'TaquerÃ', 'Taquerí')
    where name LIKE '%TaquerÃ%';
...
CREATE TABLE restaurant_tmp (
  ...
  CONSTRAINT FOREIGN KEY (version_id)
    REFERENCES restaurant_version (id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
update restaurant_version set description=replace(description,'â€', '"'),
    html_description=replace(html_description,'â€œ', '"')
    where description like '%â€%';

DELETE l FROM location l LEFT JOIN restaurant r ON l.restaurant_id=r.id
    WHERE r.id IS NULL;
...
CREATE TABLE location_tmp (
  ...
  KEY  (restaurant_id),
  KEY  (country_id),
  CONSTRAINT FOREIGN KEY (restaurant_id)
...
"""
    ...
    update menu_item_image
        set image = CONCAT('images/menuitems/',md5sum, '.jpg');
