...
def setup_ubuntu():
    ...
    users = {'davedash': "Dave Dash",
    'builder': "Build Script", 'mealadvisor': "Meal Advisor"}

    for user, fullname in users.iteritems():
        sudo('useradd -G users,sudo -m -c "%s" -s /bin/bash %s'
        % (fullname, user))
    ...
    # # PASTE:
    # ssh-rsa
    # AAAAB3NzaC1yc2EAAAABIwAAAQEAzHDofB7grcL09XY8/kWhh3zCHZdc057d/2rChWiHX
    # OeBEIKKxpAaB0V0m1sIcXnOgRGRl/Y7ENEC1xnyXPOqCdgS3MODQBJmQDPPfJLNXgkzz1
    # qBVgN+eVj4VffLd3Gwo13Q/HOwORG3l/sTj7xYzCq6iErR9iFQzZJRNsE0WvyX+zs5aOA
    # 6+nB74TmTPqY8PtGpK4yy96JjvUfdfleZ5u6zanZ8GZVKqY5Yser4Mzgsfy54DPTqDirX
    # 7a6RxYsoCP4yOIkX3/QLsrq3uJYDD1iFY2ctdHkPU/K9Bx+GCBgFSnM4H3IeXsKtdeZmu
    # +UPxLTSz+xyzR/DDrtQprQP4w== dash@awesomepants
    #
    ...
    #
    # $ sudo vim /etc/postfix/virtual# Add one USER@VIRTUAL_DOMAIN \
    # EMAIL_TO_FORWARD_TO per line, e.g.:user@example.com user@gmail.com$ \
    # sudo postmap /etc/postfix/virtual$ sudo vim /etc/aliases# Append the \
    # following line so that root mail goes to USER@DOMAINroot: USER$ sudo \
    # newaliases# Restart postfix for changes to take effect
...
def concat_minify_js():
    # concat js
    local('cat static/js/main.js static/js/home.js static/js/restaurant.js '
    + '> static/js/ma-all.js', abort='fail')
    # run compressor
    # save output to ma-min.js
    local('java -jar bin/yuicompressor-2.4.2.jar -v static/js/ma-all.js -o '
    + 'static/js/ma-min.js', abort='fail')


def concat_minify_css():
    # concat js
    local('cat static/css/main.css static/css/home.css '
    + 'static/css/restaurant.css static/css/search.css '
    + '> static/css/ma-all.css', abort='fail')
    # run compressor
    # save output to ma-min.js
    local('java -jar bin/yuicompressor-2.4.2.jar -v static/css/ma-all.css '
    +'-o static/css/ma-min.css', abort='fail')


def setup_nginx():
    sudo("ln -s %(releases_path)s/staging/config/wallace.mealadvisor.us.nginx"
    + " /etc/nginx/sites-enabled/wallace.mealadvisor.us")
...
def setup_staging():
    ...
    + "/a/mealadvisor.us/lib/python2.5/site-packages/tagging")
...
#   LOCK TABLES ON `ma_staging`.* TO 'ma_staging'@'%'
...
def pep8():
    ...
    + ".DS_Store,README,stemmer.py,manage.py"
